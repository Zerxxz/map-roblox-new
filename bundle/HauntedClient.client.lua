--[[
	HauntedClient.client.lua
	============================================================
	SATU file LocalScript untuk Client. Cara pakai:
	1. Di Roblox Studio, buka StarterPlayer -> StarterPlayerScripts.
	2. Klik kanan -> Insert Object -> LocalScript.
	3. Rename jadi "HauntedClient" (LocalScript, BUKAN Script).
	4. Paste seluruh isi file ini ke dalamnya.
	5. Play. Lihat Output (View -> Output).
	============================================================
]]
...
print("[HauntedClient] mulai")

-- Tunggu Remotes dibuat oleh server
local remotes = ReplicatedStorage:WaitForChild("Remotes", 10)
if not remotes then
	warn("[HauntedClient] Remotes folder tidak ditemukan. Pastikan HauntedServer script aktif.")
	return
end
local function w(name) return remotes:WaitForChild(name, 10) end
local R = {
	QuestUpdate      = w("QuestUpdate"),
	QuestComplete    = w("QuestComplete"),
	TriggerJumpscare = w("TriggerJumpscare"),
	HunterNear       = w("HunterNear"),
	PuzzlePrompt     = w("PuzzlePrompt"),
	PuzzleSubmit     = w("PuzzleSubmit"),
	AllianceInvite   = w("AllianceInvite"),
	AllianceRespond  = w("AllianceRespond"),
	AllianceRequest  = w("AllianceRequest"),
	AllianceUpdate   = w("AllianceUpdate"),
	AllianceLeave    = w("AllianceLeave"),
	AtmosphereCue    = w("AtmosphereCue"),
	Notify           = w("Notify"),
}

local LANTERN_TOTAL = 5
local INVITE_TIMEOUT = 20
local JUMPSCARE_SOUND_ID = "rbxassetid://12222030"
local WHISPER_SOUND_ID   = "rbxassetid://5591815081"
...
local questStroke = Instance.new("UIStroke", questFrame)
questStroke.Color = Color3.fromRGB(180, 150, 80)
questStroke.Thickness = 1.5
...
questProgress.Text = "0 / " .. LANTERN_TOTAL
...
R.QuestUpdate.OnClientEvent:Connect(function(info)
	if typeof(info) ~= "table" then return end
	questProgress.Text = string.format("%d / %d", info.collected or 0, info.total or LANTERN_TOTAL)
end)
R.QuestComplete.OnClientEvent:Connect(function()
	questTitle.Text = "QUEST SELESAI!"
	questProgress.Text = "Kamu menemukan semua Lampu Kuno!"
	questStroke.Color = Color3.fromRGB(120, 255, 150)
end)
...
R.Notify.OnClientEvent:Connect(function(text, color)
	if typeof(text) == "string" then showToast(text, color) end
end)
...
jumpscareImg.Image = "rbxassetid://7743875961"
...
jumpscareSound.SoundId = JUMPSCARE_SOUND_ID
jumpscareSound.Volume = 1.6
jumpscareSound.Parent = SoundService

R.TriggerJumpscare.OnClientEvent:Connect(function()
...
-- HUNTER NEAR
-- ============================================================
R.HunterNear.OnClientEvent:Connect(function()
...
whisperSound.SoundId = WHISPER_SOUND_ID
whisperSound.Volume = 0.6
whisperSound.Parent = SoundService

R.AtmosphereCue.OnClientEvent:Connect(function(cue)
	if cue == "whisper" then whisperSound:Play() end
end)

-- ============================================================
-- PUZZLE UI
-- ============================================================
R.PuzzlePrompt.OnClientEvent:Connect(function(puzzleId, hints)
	if typeof(puzzleId) ~= "string" or typeof(hints) ~= "table" then return end
	...
	hintBox.Text = "Petunjuk dari ruangan:\n\n- " .. table.concat(hints, "\n- ")
	...
	submit.MouseButton1Click:Connect(function()
		...
		local success, res = pcall(function()
			return R.PuzzleSubmit:InvokeServer(puzzleId, input.Text)
		end)
...
local aStroke = Instance.new("UIStroke", allianceFrame)
aStroke.Color = Color3.fromRGB(120, 180, 255)
aStroke.Thickness = 1.5

local aTitle = Instance.new("TextLabel")
aTitle.Size = UDim2.new(1, -20, 0, 22)
aTitle.Position = UDim2.new(0, 10, 0, 4)
aTitle.BackgroundTransparency = 1
aTitle.Text = "ALIANSI"
aTitle.Font = Enum.Font.GothamBold
aTitle.TextSize = 14
aTitle.TextColor3 = Color3.fromRGB(180, 220, 255)
aTitle.TextXAlignment = Enum.TextXAlignment.Left
aTitle.Parent = allianceFrame
...
inviteBtn.MouseButton1Click:Connect(function()
	if inviteBox.Text ~= "" then
		R.AllianceRequest:FireServer(inviteBox.Text)
		inviteBox.Text = ""
	end
end)
leaveBtn.MouseButton1Click:Connect(function() R.AllianceLeave:FireServer() end)

R.AllianceUpdate.OnClientEvent:Connect(function(info)
	if typeof(info) ~= "table" then return end
...
R.AllianceInvite.OnClientEvent:Connect(function(info)
	if typeof(info) ~= "table" then return end
	...
	accept.MouseButton1Click:Connect(function()
		R.AllianceRespond:FireServer({ fromUserId = info.fromUserId, accept = true })
		dlg:Destroy()
	end)
	decline.MouseButton1Click:Connect(function()
		R.AllianceRespond:FireServer({ fromUserId = info.fromUserId, accept = false })
		dlg:Destroy()
	end)
...
task.delay(2, function()
	showToast("Kontrol: [Klik senter] nyala/mati. Temukan 5 Lampu Kuno, hindari hantu.", Color3.fromRGB(255, 220, 160))
end)

print("[HauntedClient] UI siap")
